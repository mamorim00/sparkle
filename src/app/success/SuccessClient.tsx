...
export default function SuccessClient() {
    ...
    return (
      <div className="max-w-xl mx-auto p-6 text-center">
        <div className="animate-pulse">
          <h1 className="text-2xl font-bold text-gray-600 mb-4">{t('successPage.processingBooking')}</h1>
          <p className="mb-2">{t('successPage.pleaseWait')}</p>
  ...
  if (!session) {
    return <p className="text-center text-red-500">{t('successPage.couldNotRetrieve')}</p>;
  }

  if (bookingError) {
    return (
      <div className="max-w-xl mx-auto p-6 text-center">
        <h1 className="text-2xl font-bold text-green-600 mb-4">{t('successPage.paymentSuccessful')} 🎉</h1>
        <p className="mb-2">
          {t('successPage.thankYouPayment')} <b>{session.metadata?.guestName || session.customer_details?.name || ""}</b>!
        </p>
        <p className="mb-4 text-yellow-600">
          {bookingError}
        </p>
        <p className="mb-2">
          {t('successPage.receivedPayment')} <b>{(session.amount_total / 100).toFixed(2)} {session.currency.toUpperCase()}</b>.
        </p>
        <p className="mb-2">
          {t('successPage.confirmationSent')} <b>{session.customer_details?.email}</b>.
        </p>
        {session.receipt_url && (
          <a
  ...
  return (
    <div className="max-w-xl mx-auto p-6 text-center">
      <h1 className="text-2xl font-bold text-green-600 mb-4">{t('successPage.paymentSuccessful')} 🎉</h1>
      <p className="mb-2">
        {t('successPage.thankYouBooking')} <b>{session.metadata?.guestName || session.customer_details?.name || ""}</b>!
      </p>
      <p className="mb-2">
        {t('successPage.receivedPayment')} <b>{(session.amount_total / 100).toFixed(2)} {session.currency.toUpperCase()}</b>.
      </p>
      ...
      {booking && (
        <div className="mt-6 p-4 bg-gray-100 rounded-lg">
          <h2 className="text-xl font-semibold mb-3">{t('successPage.bookingDetails')}</h2>
          <div className="text-left space-y-2">
            <p><strong>{t('checkoutPage.cleaner')}:</strong> {booking.cleanerName}</p>
            <p><strong>{t('checkoutPage.service')}:</strong> {booking.cleaningType}</p>
            <p><strong>{t('checkoutPage.date')}:</strong> {booking.date}</p>
            <p><strong>{t('checkoutPage.time')}:</strong> {booking.start} - {booking.end}</p>
            <p><strong>{t('successPage.status')}:</strong> <span className="text-green-600 font-semibold">{booking.status}</span></p>
          </div>
        </div>
      )}
